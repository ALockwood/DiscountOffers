...
//NOTE: This was taken directly from https://github.com/antifriz/hungarian-algorithm-n3
//I (Andrew Lockwood) make no claim to have written this and this is me giving full attribution to the author.
//The only changes I made were to allow for double values to be passed in and maximized.
//Given how useful this is and how fun this challenge was I will likely clean this up and post on my own github for other to use, linking to the original.
//I suspect 'antifriz' ported this from here: https://www.topcoder.com/community/data-science/data-science-tutorials/assignment-problem-and-hungarian-algorithm/ 
//I will not be commenting or revising this code, other than to state that testing reveals it to be a functional implementation of "The Hungarian Algorithm" as described here:
...
namespace GraphAlgorithms
{
    public sealed class HungarianAlgorithm
    {
            ...
            while (_maxMatch != _n)
            {
                ...
                while (true)
                {
                    while (q.Count != 0)
                    {
                        ...
                        for (y = 0; y < _n; y++)
                        {
                            if (_costMatrix[x, y] != lxx + _ly[y] || _t[y])
                                continue;
                            if (_matchY[y] == -1)
        ...
        private void InitLbls()
        {
            ...
            for (var j = 0; j < _n; j++)
            {
                double minColumn = _costMatrix[0, j] - _lx[0];
                for (var i = 0; i < _n; i++)
                {
        ...
        private void AddToTree(int x, int prevx)
        {
            ...
            for (var y = 0; y < _n; y++)
            {
                if (_costMatrix[x, y] - lxx - _ly[y] >= _slack[y])
                    continue;
